	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($_POST['order_info'])) {
			...
			if($name != "" && $addr != "" && $email != "" && $phone != "" && $food != "" && $price != "") {
				...
				$insert = $db->query("INSERT INTO basket(customer_name, contact_number, adress, email, total, status, date_made) VALUES('".$name."', '".$phone."', '".$addr."', '".$email."', '".$price."', 'pending', NOW(), - INTERVAL 1 HOUR)");
